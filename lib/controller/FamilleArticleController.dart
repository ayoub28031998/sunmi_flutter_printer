...
class FamilleArticleController extends GetxController{
  String urlApi=GlobalParams().urlApi+"FamilleArticles/";
  Future <List<FamilleArticleDTO>> getFamilleArticle() async {
    final response = await http.get(Uri.parse( this.urlApi));
    if (response.statusCode == 200) {
      final List<dynamic> familleArticleJson = json.decode(utf8.decode(response.bodyBytes));
      print(familleArticleJson.length);
      final familleArticle = familleArticleJson.map((json) => FamilleArticleDTO.fromJson(json)).toList();
